            ...
            <div class="space-y-4">
                ...
                <div class="flex flex-wrap gap-3">
                    @foreach ($record->colors as $color)
                        <div class="relative">
                            <input type="radio" wire:model.live="selectedColor" value="{{ $color->id }}" required
                                class="peer sr-only"
                                {{ $color->id == $record->colors->first()->id ? 'checked' : '' }} />
                            <div class="w-10 h-10 rounded-full cursor-pointer transition-all duration-300 
            ...
            <div class="space-y-4">
                ...
                <div class="flex flex-wrap gap-3">
                    @foreach ($record->sizes as $size)
                        <div class="relative">
                            <input type="radio" wire:model.live="selectedSize" value="{{ $size->id }}" required
                                class="peer sr-only"
                                {{ $size->id == $record->sizes->first()->id ? 'checked' : '' }} />
                            <label class="flex items-center justify-center w-12 h-12 rounded-lg cursor-pointer
            ...
            <div class="space-y-4">
                ...
                <div class="flex items-center border-2 border-gray-200 rounded-lg overflow-hidden w-full max-w-xs">
                    <button type="button"
                        ...
                        wire:click="$set('quantity', {{ max($record->minimum_quantity, $quantity - 1) }})">
                        −
                    </button>
                    ...
                    <button type="button"
                        class="px-6 py-3 bg-gray-50 text-[#2B3467] hover:bg-gray-100 transition-colors duration-300"
                        wire:click="$set('quantity', {{ min($record->maximum_quantity, $quantity + 1) }})">
                        +
                    </button>
                </div>
